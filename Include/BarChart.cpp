...
void CBarChart::SetMaxValue(double dValue)
{
	m_bars.SetMaxValue(dValue);
}
...
//						  For last Add bar of the chart, so the chart won't be forced to
//						  Show changes every time a single bar is added.
void CBarChart::AddBar(int udValue, CString szLabel, COLORREF color, BOOL bRepaint)
{
	m_bars.AddBar(udValue, szLabel, color);
...
void CBarChart::DrawTitle(CDC* pDC)
{
	...
	pDC->SetTextColor(RGB(255, 210, 50));
...
void CBarChart::CChartBar::CreateLabelFont()
{
	...
	_tcsnccpy_s(m_lf.lfFaceName, _T("Tahoma"), _tcslen(_T("Tahoma")));
	m_lf.lfHeight=-10;
	m_lf.lfWidth=0;
	m_lf.lfEscapement = 0;
...
void CBarChart::CChartBar::SetMaxValue(double dValue )
{
	m_chartData.SetMaxValue(dValue);	
}
...
void CBarChart::CChartBar::AddBar(int udValue,CString szLabel, COLORREF color)
{
	m_chartData.Add(szLabel, udValue, color);
	m_sBarPicSize.cx = m_chartData.GetCount() * (m_nBarWidth + m_nBarGap);
}
...
void CBarChart::CChartBar::DrawBarsHistogram(CDC* pDC, CRect& rcBound)
{
	double nHeight = 0;
    int r1,g1,b1;
    int r2,g2,b2;
	...
	for (int i = 0; i<m_chartData.GetCount();i++)
	{
		...
		if (m_bShowBarText || m_bShowLabel)
		{
			...
			if (m_bShowBarText)
			{
				...
				if (m_nBarTextType== 0 )
				{
					str.Format(_T("%0.01f%%"), (m_chartData.GetValue(i)*100) / (int)m_chartData.GetMaxValue());
					pDC->DrawText(str, CRect(x, y - BAR_TXT_PERCENT_HEIGHT, x+ m_nBarWidth + m_nBarGap-1, y),
						DT_LEFT | DT_SINGLELINE | DT_VCENTER);
				}
				else if (m_nBarTextType==1)		// Draw value at the top of the bar
				{
					str.Format(_T("%5d"), (int)m_chartData.GetValue(i));
					pDC->DrawText(str, CRect(x, y - BAR_TXT_PERCENT_HEIGHT, x+ m_nBarWidth+ m_nBarGap-1, y),
						DT_LEFT | DT_SINGLELINE | DT_VCENTER);
				}
			}
										  ...
										  CRect& rcMemPic, BOOL bScale, CString& szTip)
{
	...
	double  dMulValue = 0.0;

	dMulValue = m_chartData.GetMaxValue();
	...
	else
	{
		szTip.Format(_T("%s\n%0.02f\n%0.02f%%\n"),
			...
			(m_chartData.GetValue((int)m_dIndex)*100)/dMulValue);
	}
}
...
int CBarChart::CChartData::SetMaxValue(double dValue)
{
	m_udMax = dValue;
...
BOOL CChartTip::Create(CWnd *pParentWnd, BOOL *pbEnable, CString* lpszTitle, int nDelay)
{
	...
	_tcsnccpy_s(lf.lfFaceName, _T("Tahoma"), _tcslen(_T("Tahoma")));
...
BOOL CBarChart::CChartDatabase::Prepare(ULONG ulODBCVer)
{
	...
	m_sqlRet = SQLSetEnvAttr(m_hOdbcEnv, SQL_ATTR_ODBC_VERSION, (SQLPOINTER)ulODBCVer, SQL_IS_INTEGER);
	if(m_sqlRet != SQL_SUCCESS && m_sqlRet != SQL_SUCCESS_WITH_INFO)
	{
...
BOOL CBarChart::CChartDatabase::Connect(CString szDSN, CString szUsername, CString szPass)
{
	...
	m_sqlRet = SQLConnect(m_hDbConn, (TCHAR *)(LPCTSTR)szDSN, SQL_NTS,
		(TCHAR *)(LPCTSTR)szUsername, SQL_NTS,
		(TCHAR *)(LPCTSTR)szPass, SQL_NTS);
...
BOOL CBarChart::CChartDatabase::ExecuteSQL(CString& szQuery)
{
    ...
    m_sqlRet = SQLExecDirect(m_hstmt, (TCHAR*)(LPCTSTR)szQuery, SQL_NTS);
    if(m_sqlRet != SQL_SUCCESS && m_sqlRet != SQL_SUCCESS_WITH_INFO)
	{
