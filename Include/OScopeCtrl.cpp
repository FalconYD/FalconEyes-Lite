...
{
	int i;
	static const COLORREF PresetColor[34] = 
	{
		RGB(0xFF, 0xFF, 0xFF),
		RGB(0xFF, 0x00, 0x00),
		RGB(0x00, 0xFF, 0x00),
...
void COScopeCtrl::DrawGraph(BYTE* pData, int nSize, int iTrend)
{
	...
	if (m_dcPlot.GetSafeHdc() != NULL)
	{
		...
		prevY = static_cast<int>(m_rectPlot.bottom - pData[0] * m_PlotData[iTrend].dVerticalFactor);
		m_dcPlot.MoveTo(prevX, prevY);
		for (int i = 1; i < nSize; i++)
		{
			currX = static_cast<int>(prevX + i / (double)nSize * m_rectPlot.Width());
			currY = static_cast<int>(m_rectPlot.bottom - pData[i] * m_PlotData[iTrend].dVerticalFactor);
